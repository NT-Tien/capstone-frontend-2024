import { FixRequestStatus } from "@/lib/domain/Request/RequestStatus.enum"
import { cn } from "@/lib/utils/cn.util"
import { Tag } from "antd"

type Props = {
   status?: FixRequestStatus
   className?: string
}

function RequestStatusTag(props: Props) {
   switch (props.status) {
      case FixRequestStatus.PENDING:
         return (
            <span
               className={cn(
                  "inline-flex items-center gap-x-1.5 rounded-md bg-gray-100 px-2 py-1 text-xs font-medium text-gray-600",
                  props.className,
               )}
            >
               <svg className="h-1.5 w-1.5 fill-gray-400" viewBox="0 0 6 6" aria-hidden="true">
                  <circle cx={3} cy={3} r={3} />
               </svg>
               Chưa xử lý
            </span>
         )
      case FixRequestStatus.APPROVED:
      case FixRequestStatus.IN_PROGRESS:
         return (
            <span
               className={cn(
                  "inline-flex items-center gap-x-1.5 rounded-md bg-blue-100 px-2 py-1 text-xs font-medium text-blue-700",
                  props.className,
               )}
            >
               <svg className="h-1.5 w-1.5 fill-blue-500" viewBox="0 0 6 6" aria-hidden="true">
                  <circle cx={3} cy={3} r={3} />
               </svg>
               Đang sửa chữa
            </span>
         )
      case FixRequestStatus.HEAD_CONFIRM:
         return (
            <span
               className={cn(
                  "inline-flex items-center gap-x-1.5 rounded-md bg-yellow-100 px-2 py-1 text-xs font-medium text-yellow-800",
                  props.className,
               )}
            >
               <svg className="h-1.5 w-1.5 fill-yellow-500" viewBox="0 0 6 6" aria-hidden="true">
                  <circle cx={3} cy={3} r={3} />
               </svg>
               Chờ đánh giá
            </span>
         )
      case FixRequestStatus.CLOSED:
         return (
            <span
               className={cn(
                  "inline-flex items-center gap-x-1.5 rounded-md bg-green-100 px-2 py-1 text-xs font-medium text-green-700",
                  props.className,
               )}
            >
               <svg className="h-1.5 w-1.5 fill-green-500" viewBox="0 0 6 6" aria-hidden="true">
                  <circle cx={3} cy={3} r={3} />
               </svg>
               Đã đóng Badge
            </span>
         )
      case FixRequestStatus.HEAD_CANCEL:
      case FixRequestStatus.REJECTED:
         return (
            <span
               className={cn(
                  "inline-flex items-center gap-x-1.5 rounded-md bg-red-100 px-2 py-1 text-xs font-medium text-red-700",
                  props.className,
               )}
            >
               <svg className="h-1.5 w-1.5 fill-red-500" viewBox="0 0 6 6" aria-hidden="true">
                  <circle cx={3} cy={3} r={3} />
               </svg>
               Từ chối
            </span>
         )
      default:
         return (
            <span
               className={cn(
                  "inline-flex items-center gap-x-1.5 rounded-md bg-gray-100 px-2 py-1 text-xs font-medium text-gray-600",
                  props.className,
               )}
            >
               <svg className="h-1.5 w-1.5 fill-gray-400" viewBox="0 0 6 6" aria-hidden="true">
                  <circle cx={3} cy={3} r={3} />
               </svg>
               Chưa xử lý
            </span>
         )
   }
}

export default RequestStatusTag
