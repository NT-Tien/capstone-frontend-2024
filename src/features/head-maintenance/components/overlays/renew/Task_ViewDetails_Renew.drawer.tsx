...
import Task_VerifyComplete_RenewDrawer, { Task_VerifyComplete_RenewDrawerProps } from "./Task_VerifyComplete_Renew.drawer"

export type Task_ViewDetailsDrawerRenewRefType = {
...
type Props = {
   ...
   autoCreateTaskFn?: () => Promise<void>
}

const Task_ViewDetailsDrawerRenew = forwardRef<Task_ViewDetailsDrawerRenewRefType, Props>(
   function Component(props, ref) {
      ...
      const control_taskVerifyComplete_renewDrawer = useRef<RefType<Task_VerifyComplete_RenewDrawerProps>>(null)
      ...
      function handleOpenTaskVerifyComplete() {
         ...
         if (isRenewTask) {
            control_taskVerifyComplete_renewDrawer.current?.handleOpen({ task: api_task.data })
            return
         }
      ...
      const isInstalledTask = useMemo(() => {
         if (!api_task.isSuccess) return
         return !!api_task.data.issues.find((issue) => issue.typeError.id === NewDeviceInstallation)
      }, [api_task.data?.issues, api_task.isSuccess])

      const isRemovedTask = useMemo(() => {
         if (!api_task.isSuccess) return
         return !!api_task.data.issues.find((issue) => issue.typeError.id === RemoveOldDeviceTypeErrorId)
      }, [api_task.data?.issues, api_task.isSuccess])

      const isRenewTask = useMemo(() => {
         return isInstalledTask || isRemovedTask
      }, [isInstalledTask, isRemovedTask])
      ...
      return (
         <>
                     ...
                     >
                        ...
                        <h2 className="w-full text-wrap break-words align-middle font-bold leading-[18px]">
                           ...
                           <div className="mt-2 flex items-center text-base font-normal">
                              ...
                              {isRenewTask && <Tag color="orange-inverse">Bảo hành</Tag>}
                           </div>
                        </h2>
                     </div>
            ...
            <OverlayControllerWithRef ref={control_taskVerifyComplete_renewDrawer}>
               <Task_VerifyComplete_RenewDrawer onSubmit={handleUpdateConfirmCheck} />
            </OverlayControllerWithRef>
...
export default Task_ViewDetailsDrawerRenew
