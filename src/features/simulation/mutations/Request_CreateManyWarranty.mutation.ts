...
export default function useRequest_CreateManyWarranty(props?: Props) {
   ...
   return useCustomMutation({
      ...
      mutationFn: async (req: Request) => {
         ...
         let deviceList = allDevices.list.filter((device) => {
            if (!device.machineModel.warrantyTerm) return false
            if (ignoreDevices.has(device.id)) return false
            return dayjs(device.machineModel.warrantyTerm).isAfter(dayjs())
         })

         if (deviceList.length < req.count) {
            throw new Error(`Không đủ thiết bị để tạo yêu cầu (còn ${deviceList.length})`)
         }
      ...
      mutationKey: ["simulation", "request", "create-many", "warranty"],
      messages: {
         error: (error) => {
            if (error instanceof Error && error.message.startsWith("Không đủ thiết bị để tạo yêu cầu")) {
