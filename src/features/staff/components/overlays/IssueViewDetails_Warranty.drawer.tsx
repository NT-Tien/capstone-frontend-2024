...
import { App, Button, Card, Descriptions, Drawer, DrawerProps, Dropdown, Empty, Image, List, Segmented } from "antd"
import {
   BookOutlined,
   CheckOutlined,
   CloseOutlined,
...
import { ChartDonut, Factory, File, FileText, Gear, MapPin, SealWarning, Truck } from "@phosphor-icons/react"
...
} from "@/features/staff/components/overlays/ResolveIssue_Warranty.drawer"
               ...
               <div className={"flex items-center gap-2"}>
                  <Button
                     ...
                     onClick={() => {
                        ...
                        if (props.issue.typeError.id === SendWarrantyTypeErrorId) {
                        ...
                        if (props.issue.typeError.id === ReceiveWarrantyTypeErrorId) {
                           control_issueResolveDrawer.current?.handleOpen({
                              issue: props.issue,
                           })
                           return
                        }
                        if (props.issue.typeError.id === AssembleDeviceTypeErrorId) {
                        ...
                        modal.confirm({
                           title: "Lưu ý",
                           content: "Bạn có chắc chắn muốn hoàn thành bước này không?",
                           centered: true,
                           maskClosable: true,
                           onOk: () => props.issue && handleResolveIssue(props.issue.id),
                        })
                     }}
                     icon={<CheckOutlined />}
