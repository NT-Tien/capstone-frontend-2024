...
type Props = Omit<DrawerProps, "children"> & TaskViewDetails_RenewDrawerProps & { handleClose?: () => void }

function TaskViewDetails_RenewDrawer(props: Props) {
   ...
   const renewDevice = useMemo(() => {
      if (!api_task.isSuccess || !api_task.data?.device_renew) return

      const deviceArray = Array.isArray(api_task.data.device_renew)
   ...
   }, [api_task.data?.device_renew, api_task.isSuccess])
   // const qrCode = api_task.data?.id ?? ""
   // const isDeviceRenewAvailable = api_task.data?.device_renew !== null
   // const handleGetRenewDevice = () => {
   //    const renewDevice = api_task.data?.device_renew

   //    if (renewDevice && control_qrCodeDisplayForRenewModal.current) {
   //       control_qrCodeDisplayForRenewModal.current?.handleOpen(qrCode, renewDevice)
   //    }
   // }
   ...
   function Footer() {
      ...
      if (api_task.data?.status === TaskStatus.IN_PROGRESS) {
         return (
            <Button
               ...
               type={"primary"}
               size={"large"}
               onClick={() => api_task.isSuccess && router.push(staff_uri.stack.task_id_renew(api_task.data.id))}
            >
               Tiếp tục tác vụ
            </Button>
         )
      ...
      // if (
      //    hasTakenRenewDevice === false &&
      //    api_task.data?.export_warehouse_ticket.forEach(
      ...
      //                api_task.isSuccess &&
      //                renewDevice &&
      //                control_qrCodeDisplayForRenewModal.current?.handleOpen(api_task.data.id, renewDevice)
      //             }
      //          >
      //             Lấy thiết bị
      //          </Button>
      ...
      // }
      // {
      //    isDeviceRenewAvailable && (
      //       <Button
      //          block
      //          type="primary"
      //          size="large"
      //          onClick={() => {
      //             handleGetRenewDevice
      //          }}
      //       >
      //          Lấy thiết bị
      //       </Button>
...
export default TaskViewDetails_RenewDrawer
export type { TaskViewDetails_RenewDrawerProps }
